...
interface GameSessionDao {
    ...
    @Transaction
    @Query(
        "SELECT * FROM game_sessions WHERE user_id = :userId ORDER BY started_at DESC LIMIT :limit"
    )
    suspend fun recentForUser(userId: String, limit: Int = 10): List<GameSessionWithUser>
}
