...
class GetQuestionsUseCaseTest {
    ...
    @Test
    fun invoke_withoutLocalQuestions_fetchesRemotePersistsAndReturnsMappedRemote() = runTest {
        val remote = listOf(
            RemoteQuestion(
                id = "q2",
                question = "Pregunta 2",
        ...
        whenever(localRepository.getAll()).thenReturn(emptyList())
        ...
        assertEquals(remote.map { it.toModel() }, result)
    }
    ...
    private fun RemoteQuestion.toModel() = QuestionModel(
        id = id,
        question = question,
        category = category,
